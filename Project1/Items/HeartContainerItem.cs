...
{
    class HeartContainerItem : IGameObject
    {
        ...
        public HeartContainerItem(Texture2D texture, Vector2 position)
        {
            Exists = true;
            _sprite = new HeartContainerSprite(texture);
            _point = new ItemPoint(position);
            SetRectangle();
            ObjectProperty = ObjectType.Item;
            SoundProperty = SoundType.Heart;
        }
