...
{
    class TriforcePieceItem : IGameObject
    {
        ...
        public TriforcePieceItem(Texture2D texture, Vector2 position)
        {
            Exists = true;
            _sprite = new TriforcePieceSprite(texture);
            _point = new ItemPoint(position);
            SetRectangle();
            ObjectProperty = ObjectType.Item;
            SoundProperty = SoundType.Triforce;
        }
