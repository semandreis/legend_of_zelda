...
{
    class GoriyaObject : IGameObject
    {
        ...
        public GoriyaObject (Vector2 position)
        {
            _sprite = NPCSpriteFactory.Instance.CreateGoriyaDownSprite();
            _point = new GoriyaPoint(position);
            ...
            ((GoriyaPoint)_point).GoriyaDirection = Direction.Down;
            Exists = true;
            ...
            _moving = true;
            SetRectangle();
            EnemyProperty = Enemy.Goriya;
        }

        public void Draw(GameTime gameTime, SpriteBatch sb)
        {
            _sprite.Draw(_point.ObjectPoint, NPCConstants.GoriyaFrameTime, gameTime, NPCConstants.EnemyResize, sb);
            _previousPoint = _point.ObjectPoint;
        }
        ...
        private void CreateSpriteDirection()
        {
            if (_num == 0)
            {
                _sprite = NPCSpriteFactory.Instance.CreateGoriyaRightSprite();
                ((GoriyaPoint)_point).GoriyaDirection = Direction.Right;
            }
            else if (_num == 1)
            {
                _sprite = NPCSpriteFactory.Instance.CreateGoriyaLeftSprite();
                ((GoriyaPoint)_point).GoriyaDirection = Direction.Left;
            }
            else if (_num == 2)
            {
                _sprite = NPCSpriteFactory.Instance.CreateGoriyaDownSprite();
            ...
            {
                _sprite = NPCSpriteFactory.Instance.CreateGoriyaUpSprite();
                ((GoriyaPoint)_point).GoriyaDirection = Direction.Up;
            }
            else
            {
                ((GoriyaPoint)_point).GoriyaDirection = Direction.None;
            }
        }
        ...
        private void ChangeDirection(GameTime gameTime)
        {
            ...
            if (_timeSinceMovement > NPCConstants.GoriyaMovementTime)
            {
                _num = _random.Next(5);
                _moving = false;
                CreateSpriteDirection();
                _timeSinceMovement -= NPCConstants.GoriyaMovementTime;
            }
            else
            {
                _moving = true;
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (_moving)
            {
                _point.Move(NPCConstants.GoriyaVelocity, gameTime);
            }
            SetRectangle();
        }
        ...
        public void Initial()
        {
            _point = new GoriyaPoint(_initialPoint);
            SetRectangle();
        }
    }
}
