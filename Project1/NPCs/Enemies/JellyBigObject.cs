...
{
    class JellyBigObject : IGameObject
    {
        ...
        public JellyBigObject (Vector2 position)
        {
            _sprite = NPCSpriteFactory.Instance.CreateJellyBigSprite();
            _point = new JellyBigPoint(position);
            if (Globals.HardMode)
            {
        ...
        public void Initial()
        {
            _point = new JellyBigPoint(_initialPoint);
            SetRectangle();
        }
    }
}
