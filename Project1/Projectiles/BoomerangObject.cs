...
{
    class BoomerangObject : IGameObject
    {
        ...
        public BoomerangObject(Texture2D texture, Vector2 position, Direction direction)
        {
            _point = new BoomerangPoint(position, direction, ProjectileConstants.BoomerangAcceleration);
            _lifespan = ProjectileConstants.BoomerangLifespan;
            _timer = 0;
            _velocity = ProjectileConstants.BoomerangVelocity;
            _sprite = new BoomerangSprite(texture, direction);
            Exists = true;
            SetRectangle();
