...
{
    class MagicalSwordObject : IGameObject
    {
        ...
        public MagicalSwordObject(Texture2D texture, Vector2 position, Direction direction)
        {
            _point = new SwordPoint(position, direction, ProjectileConstants.SwordAcceleration);
            _lifespan = ProjectileConstants.SwordLifespan;
            _timer = 0;
            _velocity = ProjectileConstants.SwordVelocity;
            _sprite = new MagicalSwordSprite(texture, direction, _lifespan);
            Exists = true;
            SetRectangle();
